...
export default function ProjectLab({
  ...
  const run = async () => {
    ...
    try {
      ...
      const compiled = ts.transpileModule(code, { compilerOptions:{target:ts.ScriptTarget.ES2020,module:ts.ModuleKind.ESNext,strict:true}, reportDiagnostics:true });
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.workspace}>
        ...
        <main className={styles.editorArea}><header><span>project.ts</span><button onClick={()=>setCopyMode((value)=>!value)}><Copy size={14}/>{copyMode?'通常演習':'写経モード'}</button><button onClick={()=>setShowSolution((value)=>!value)}><Eye size={14}/>差分・解答</button><button onClick={()=>{setCodeByProject((current)=>({...current,[project.id]:copyMode?'':project.starterCode}));setResult(null)}}><RotateCcw size={14}/>リセット</button><button className={styles.run} disabled={running} onClick={()=>void run()}>{running?<Loader2 className={styles.spin} size={14}/>:<Play size={14}/>}実行</button></header>{copyMode&&<div className={styles.copyGuide}><Code2 size={15}/>模範コードを一度読み、見ずに再現します。詰まったら「差分・解答」を短時間だけ開きましょう。</div>}<Editor height="560px" language="typescript" theme="vs-dark" value={code} beforeMount={configureLearningTypeScript} onChange={(value)=>{const next=value||'';setCodeByProject((current)=>({...current,[project.id]:next}));persist(next)}} options={{fontSize:14,lineHeight:23,minimap:{enabled:false},automaticLayout:true,wordWrap:'on',padding:{top:16,bottom:16}}}/></main>
      </div>
