...
const seeds: FullstackSeed[] = [
  ...
  { id:'capstone-release',title:'卒業制作3: テスト・公開・改善',chapter:'Chapter 16 卒業制作',category:'卒業制作',level:'advanced',minutes:100,summary:'テスト、CI、デプロイ、監視まで完結させます。',concept:'主要導線のテスト、環境変数、migration、デプロイ、公開URL確認、ログ確認、バックアップ方針まで揃えて初めて運用可能なWebアプリになります。',syntax:'test -> migrate -> deploy -> smoke test -> monitor',code:'type ReleaseCheck={name:string;passed:boolean};\nconst checks:ReleaseCheck[]=[{name:"test",passed:true},{name:"deploy",passed:true},{name:"smoke",passed:true}];\nconsole.log(checks.every((check)=>check.passed));',output:'true',focus:['リリース','migration','スモークテスト','監視'],mistake:'ローカルで動いた状態を本番完成と考える',practice:'リリース条件がすべて成功したか確認する' },
  {
    id: 'api-validation',
    title: 'API入力バリデーション',
    chapter: 'Chapter 9 API設計',
    category: 'API',
    level: 'standard',
    minutes: 30,
    summary: 'APIで受け取るデータをZodなどで型と整合性を検証します。',
    concept: 'ユーザー入力は信用せず、サーバー側で必ず検証してから使います。',
    syntax: "const data = Schema.parse(await request.json());",
    code: "import { z } from 'zod';\nconst UserSchema = z.object({ name: z.string().min(1), age: z.number().int().min(0) });\nexport async function POST(request: Request) {\n  const data = UserSchema.parse(await request.json());\n  return Response.json({ ok: true, name: data.name });\n}",
    output: "{ ok: true, name: '...' }",
    focus: ['Zod', 'バリデーション', '型ガード'],
    mistake: 'リクエストボディを as 型でキャストして信用する',
    practice: 'email 形式と必須項目を含む LoginSchema を作る',
  },
  {
    id: 'cors-config',
    title: 'CORSとプリフライト',
    ...
    level: 'standard',
    minutes: 25,
    summary: 'ブラウザから別オリジンのAPIを呼ぶためのCORS設定を学びます。',
    concept: 'OPTIONS プリフライトでサーバーが許可するかを返します。',
    syntax: "res.headers.set('Access-Control-Allow-Origin', '*');",
    code: "export function middleware(request: Request) {\n  const res = NextResponse.next();\n  res.headers.set('Access-Control-Allow-Origin', '*');\n  res.headers.set('Access-Control-Allow-Methods', 'GET,POST,OPTIONS');\n  return res;\n}",
    output: "headers set",
    focus: ['CORS', 'プリフライト', 'Next.js middleware'],
    mistake: 'Access-Control-Allow-Origin に複数のオリジンをカンマで並べる',
    practice: '環境変数で許可オリジンを切り替える middleware を作る',
  },
  {
    id: 'error-boundary',
    title: 'Reactのエラーバウンダリ',
    chapter: 'Chapter 10 堅牢なUI',
    category: 'エラー処理',
    level: 'standard',
    minutes: 25,
    summary: '子コンポーネントの例外をキャッチしてUI全体を守ります。',
    concept: 'React 18以降は ErrorBoundary を作って fallback を表示します。',
    syntax: '<ErrorBoundary fallback={<div>Error</div>}>...</ErrorBoundary>',
    ...
    output: "renders fallback on error",
    focus: ['ErrorBoundary', 'フォールバック', 'componentDidCatch'],
    mistake: 'useEffect の中で起きた同期例外を ErrorBoundary なしで放置する',
    practice: 'API呼び出し中の失敗を ErrorBoundary でハンドリングする',
  },
  {
    id: 'rsc-basics',
    title: 'React Server Components入門',
    chapter: 'Chapter 11 サーバーコンポーネント',
    category: 'RSC',
    level: 'standard',
    minutes: 35,
    summary: 'サーバー側でレンダリングするコンポーネントの基本を学びます。',
    concept: 'RSC は useState を持たず、サーバーで HTML を作って返します。',
    syntax: "export default async function Page() { const data = await fetch(...); return <div>{data}</div>; }",
    code: "export default async function Page() {\n  const r = await fetch('https://api.example.com/items', { cache: 'no-store' });\n  const items = await r.json();\n  return (<ul>{items.map((it: { id: number; name: string }) => <li key={it.id}>{it.name}</li>)}</ul>);\n}",
    output: "<ul>...</ul>",
    focus: ['RSC', 'async component', 'no-store'],
    mistake: 'RSC の中で onClick などクライアント専用イベントを使う',
    practice: 'fetch の cache 戦略を force-cache と no-store で比較する',
  },
  {
    id: 'streaming-ssr',
    title: 'Streaming SSRとSuspense',
    chapter: 'Chapter 11 サーバーコンポーネント',
    category: 'RSC',
    level: 'advanced',
    minutes: 30,
    summary: '遅いデータ部分をSuspenseで囲み、先に速い部分だけ表示します。',
    concept: 'Suspense配下のPromise解決を待ちながら他のHTMLを順次送れます。',
    syntax: '<Suspense fallback={<Loading/>}><SlowPart/></Suspense>',
    code: "import { Suspense } from 'react';\nexport default function Page() {\n  return (\n    <div>\n      <h1>Hello</h1>\n      <Suspense fallback={<p>loading...</p>}>\n        <SlowPart />\n      </Suspense>\n    </div>\n  );\n}\nasync function SlowPart() {\n  await new Promise((r) => setTimeout(r, 1000));\n  return <p>slow content</p>;\n}",
    output: "Hello / loading... -> slow content",
    focus: ['Streaming SSR', 'Suspense', 'TTFB'],
    mistake: 'Suspenseを置かずに重いデータをawait して全体ブロックする',
    practice: '複数の独立したデータをそれぞれ Suspense で分離して高速化する',
  },
];
